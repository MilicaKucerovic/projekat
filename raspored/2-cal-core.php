...
class Calendar {
  ...
  // (D) SAVE EVENT
  function save ($start, $end, $email, $txt, $color, $id=null) {
    // (D1) START & END DATE QUICK CHECK
    $uStart = strtotime($start);
    $uEnd = strtotime($end);
    if ($uEnd < $uStart) {
      $this->error = "Крајњи датум не може бити пре почетног";
      return false;
    }

    // (D2) SQL - INSERT OR UPDATE
    if ($id==null) {
      $sql = "INSERT INTO `events1` (`evt_start`, `evt_end`, `email`, `evt_text`, `evt_color`) VALUES (?,?,?,?,?)";
      $data = [$start, $end, $email, $txt, $color];
    } else {
      $sql = "UPDATE `events1` SET `evt_start`=?, `evt_end`=?, `email`=?, `evt_text`=?, `evt_color`=? WHERE `evt_id`=?";
      $data = [$start, $end, $email, $txt, $color, $id];
    }
  ...
  // (E) DELETE EVENT
  function del ($id) {
    return $this->exec("DELETE FROM `events1` WHERE `evt_id`=?", [$id]);
  }
